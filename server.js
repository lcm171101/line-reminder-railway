...
app.post("/set-reminder", async (req, res) => {
  try {
    ...
    await sheets.spreadsheets.values.append({
      ...
      requestBody: {
        values: [[
          reminder.id,
          reminder.name,
          reminder.mainCategory,
          reminder.subCategory,
          reminder.time,
          reminder.message,
          reminder.repeatType,
          reminder.repeatParam,
          reminder.expireDate,
          reminder.createdBy,
          reminder.lastSent
        ]]
    ...
    res.send("✅ 提醒已成功寫入 Google Sheets！<br><a href='/form'>返回</a>");
  } catch (err) {
    console.error("❌ Google Sheets 寫入失敗：", err.message);
...
app.listen(PORT, () => {
  console.log("✅ LINE Reminder Bot (Sheets) is running on port " + PORT);
});
